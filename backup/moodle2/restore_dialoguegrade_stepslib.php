...
class restore_dialoguegrade_activity_structure_step extends restore_activity_structure_step {
        ...
        foreach ($rs as $rec) {
            ...
            if (empty($file->repositoryid)) {
                ...
                if (!$fs->file_exists($newcontextid, $component, $filearea, $rec->newitemid, $file->filepath, $file->filename)) {
                    ...
                    $info->oldfile = $file;
                    // Newfile holds the info for the new filerecord with the context, user and itemid mapped.
    ...
    protected function build_missing_conversation_index() {
        ...
        while ($conversations) {
            ...
            $conversationindex = 0;
            $messages = $DB->get_records('dialoguegrade_messages', array('conversationid' => $conversation->id),
                                         'timecreated', 'id');
            while ($messages) {
