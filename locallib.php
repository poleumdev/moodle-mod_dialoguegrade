 ...
 * @return array()
 */
function dialoguegrade_search_potentials(\mod_dialoguegrade\dialogue $dialogue, $query = '') {
    global $DB, $USER, $PAGE;
 ...
 * @return array
 */
function dialoguegrade_get_conversation_participants(\mod_dialoguegrade\dialogue $dialogue, $conversationid) {
    global $DB;
 ...
 * @return type
 */
function dialoguegrade_get_user_details(\mod_dialoguegrade\dialogue $dialogue, $userid) {
    global $DB, $PAGE;
    static $cache;
    ...
    if (!isset($cachedusers[$userid])) {
        ...
        dialoguegrade_add_user_picture_fields($user);
 ...
 * @return mixed
 */
function dialoguegrade_get_cached_param($name, $value, $default) {
    global $PAGE;
    ...
    $cache = cache::make('mod_dialoguegrade', 'params');
 ...
 * @return int
 */
function dialoguegrade_cm_unread_total(\mod_dialoguegrade\dialogue $dialogue) {
    global $USER, $DB;
    ...
    list($insql, $inparams) = $DB->get_in_or_equal(\mod_dialoguegrade\dialogue::get_unread_states(), SQL_PARAMS_NAMED, 'un');
    ...
    $sql = "SELECT
                 (SELECT COUNT(1)
                    ...
                    JOIN {dialoguegrade_participants} dp ON dp.conversationid = dm.conversationid
                   WHERE dm.dialogueid = :todialogueid
                     AND dp.userid = :touserid
                     AND dm.state $insql) -
                 (SELECT COUNT(1)
                    FROM {dialoguegrade_flags} df
                    JOIN {dialoguegrade_participants} dp ON dp.conversationid = df.conversationid
...
function dialoguegrade_get_draft_listing(\mod_dialoguegrade\dialogue $dialogue, &$total = null) {
    global $PAGE, $DB, $USER;
    ...
    $basesql = "FROM {dialoguegrade_messages} dm
                JOIN {dialoguegrade_conversations} dc
    ...
    $params = array('dialogueid' => $dialogue->activityrecord->id,
        'state' => \mod_dialoguegrade\dialogue::STATE_DRAFT,
        'userid' => $USER->id);
 ...
 * @return string html
 */
function dialoguegrade_generate_summary_line($subject, $body, $bodyformat, $length = 70, $separator = ' - ') {
 ...
 */
function dialoguegrade_get_conversations_count($cm, $state = null) {
    global $USER, $DB;
    ...
    $states = array(\mod_dialoguegrade\dialogue::STATE_OPEN, \mod_dialoguegrade\dialogue::STATE_CLOSED);
    ...
    if (!has_capability('mod/dialoguegrade:viewany', $context)) {
        $joins[] = "JOIN {dialoguegrade_participants} dp ON dp.conversationid = dc.id ";
 ...
 * @return array
 */
function dialoguegrade_get_humanfriendly_dates($epoch) {
 ...
 */
function dialoguegrade_shorten_html($html, $ideal = 30, $exact = false, $ending = '...') {
 ...
 * @return boolean
 */
function dialoguegrade_contains_draft_files($draftid) {
    global $USER;
