...
class conversations_by_role extends conversations {
    ...
    public function setup() {
        ...
        list($instatesql, $instateparams) = $DB->get_in_or_equal(array(dialogue::STATE_OPEN,
                                                                    dialogue::STATE_CLOSED),
                                                                    SQL_PARAMS_NAMED, 'lastmessagestate');
        ...
        $this->basesql  = "FROM {user} u
                           ...
                           JOIN {dialoguegrade_participants} dp ON dp.userid = u.id
                           JOIN {dialoguegrade_conversations} dc ON dc.id = dp.conversationid
                           JOIN {dialoguegrade_messages} dm ON dm.conversationid = dp.conversationid
        ...
        if (!has_capability('mod/dialoguegrade:viewany', $this->dialogue->context)) {

            $this->basesql .= " JOIN (SELECT dp.conversationid
                                        FROM {dialoguegrade_participants} dp
    ...
    public static function get_sort_options() {
        ...
        $options = array(
                         'latest' => array('directional' => false, ),
                         'oldest' => array('directional' => false, ),
                         'fullname' => array ('directional' => true, 'type' => PARAM_ALPHA, 'default' => 'asc', ),
                         'firstname' => array('directional' => true, 'type' => PARAM_ALPHA, 'default' => 'asc', ),
                         'lastname' => array ('directional' => true, 'type' => PARAM_ALPHA, 'default' => 'asc', ),
                        );
